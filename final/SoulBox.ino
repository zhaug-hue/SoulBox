...
void handleWebCommand(const String &command) {
  if (command == "web_attack") {
    ...
    game.callBoss();
  } else if (command == "call_boss_breakfast") {
    game.callBossWithType(0);
  } else if (command == "call_boss_lunch") {
    game.callBossWithType(1);
  } else if (command == "call_boss_dinner") {
    game.callBossWithType(2);
