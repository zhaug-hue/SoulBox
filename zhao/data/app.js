...
// Goal: never let one missing field / missing Chart.js / missing DOM node stop the dashboard.
...
const modalText = {
  ...
  environmentAdvice: `Environment advice is generated by SoulBox from local sensor data and real weather data.`,
...
function connect() {
  ...
  socket.onopen = () => {
    ...
    // Safe even if ESP32 does not implement get_status. It simply helps if supported.
...
function handleModals(data) {
  ...
  if (data.showWeatherBuff) {
    return openInfo("Weather Buff", modalText.weatherBuff, `weather-${data.weatherType}`);
  }
  if (data.showEnvironmentAdvice) {
    return openInfo("Environment Advice", data.environmentAdvice || modalText.environmentAdvice, "environment");
  }
  if (data.showSatietyWarning) {
    return openInfo("Satiety Warning", modalText.satietyWarning, "satiety");
  }
}
...
function showExternalEnvironment() {
  if (!latestData) {
    openManualInfo(
      "外在環境詳細數據",
      "尚未收到 ESP32 狀態資料。請確認 WebSocket 已連線，或先按 Sync Real Weather。",
      `external-env-empty-${Date.now()}`
    );
    sendCommand("get_status");
    return;
  }
